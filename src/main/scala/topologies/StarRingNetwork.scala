...
class StarRingNetwork(val nodeNamePrefix:String, val center:String, val devices:List[String]) extends Topology{
  override def create(projectManager:ProjectManager): Unit = {
    ...
    projectManager.addLink(nodeNamePrefix+center, 0, 0, nodeNamePrefix+devices(0), 0, 0)
